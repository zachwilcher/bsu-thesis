...
Treating a graph \(\Gamma\) as a one dimensional CW complex we can construct the
\(n\) point discretized configuration space \(\DConf_n(\Gamma)\) by 
...
Abrams showed in \cite{abrams2000configurationspaces} that \(\Conf_n(\Gamma)\) deformation retracts onto \(\DConf_n(\Gamma)\) 
when \(\Gamma\) is sufficiently subdivided. In this project we will focus strictly on the combinatorial configuration space
and not elaborate on this subdivision. For the interested reader, in \cite{PRUE2014136} Abram's original subdivision
was improved upon. Furthermore, Abrams showed that \(\DConf_n(\Gamma)\) is a cubical complex.
...
points on \(\Bd(B)\).
% homeomorphisms map boundary points to boundary points, interior points to interior points, and exterior points to exterior points.
...
\begin{proof}
    ...
    Since \(C\) is a surface there exists some open ball \(B\) centered at \(p\) in the interior of \(C\) such that 
    ...
    Let \(V' = V \setminus \{q\}\) and \(B' = B \setminus \{p\}\).
    Locally, \(V'\) is a punctured plane 
    which retracts to a circle surrounding the point \(q\).
    Theorem 54.5 in \cite{Munkres2000} shows that this circle is simply connected.

    However, \(B'\) locally is a punctured \(n\)-dimensional ball which retracts to an \((n-1)\) sphere
    surrounding the point \(p\). Since \(n - 1 \ge 2\), Theorem 59.3 in \cite{Munkres2000} it is shown that
    this sphere is simply connected.
    Since local simple connectedness is a topological property, we have reached a contradiction.
\end{proof}
